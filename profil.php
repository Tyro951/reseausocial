...
if (isset($_POST['description']) and !empty($_POST['description'])) {
    ...
    $insertmsg = $bdd->prepare('INSERT INTO tweets(description, tag) VALUES(?, ?)');
    $insertmsg->execute(array($description, $tags));
...
if(isset($_POST["submit"]) && !empty($_FILES["image"]["name"])){
  ...
  if(in_array($typeFichier, $formatsAutorises)){
    ...
    if(move_uploaded_file($_FILES["image"]["tmp_name"], $cheminFichierCible)){
      ...
      $insertion = $bdd->query("INSERT into tweets(image) VALUES('".$nomFichier."')");
      $requete = $bdd->query("SELECT image FROM tweets WHERE id = ?");

      if($requete->num_rows > 0){
        while($ligne = $requete->fetch_assoc()){
            $URLImage = $repertoireCible.$ligne["image"];
          }
        }
      }
    }
...
<body class="daymode">
  ...
  <main>
    ...
    <div class="profilconnexion">
      <h3> Vous êtes connnectés en tant que :
    ...
    while ($msg = $allmsg->fetch()) { ?>
      ...
      if (isset($_SESSION['user'])) { ?>
        ...
        <?php if(isset($_POST["submit"]) && !empty($_FILES["image"]["name"])) { ?>
        <img src="<?php echo $imageURL; ?>" class="card-img-bottom" alt="onepiece"> <?php } ?>
        <div class="containerpseudophoto">
